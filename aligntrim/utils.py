...
def get_scheme(
    ...
    read_file: str = False,  # type: ignore
):
    ...
    if scheme[scheme_length][scheme_version].get("refselect"):
        ...
        scheme_select_dir = os.path.join(
            scheme_directory,
            scheme_name,
            scheme_length,  # type: ignore
            scheme_version,  # type: ignore
        )  # type: ignore
...
def choose_model(read_file: str) -> dict:
    ...
    if len(tags["basecall_model_version_id"].split("_")) == 4:
        ...
        try:
            possible_models = [x for x in possible_models if preset in x["name"]]  # type: ignore
        except NameError:
    ...
    if not possible_models:  # type: ignore
        print(
            colored.red(
                f"No model found for basecall model id {tags['basecall_model_version_id']}, please provide a model with the --model parameter",
                file=sys.stderr,
    ...
    try:
        ...
        possible_models = [x for x in possible_models if model in x["name"]]  # type: ignore
    ...
    try:
        for x in possible_models:
            if basecaller_version in x["name"]:  # type: ignore
                print(
                    colored.yellow(
...
def pick_best_ref(
    ...
    best_ref = max(ref_results, key=ref_results.get)  # type: ignore
